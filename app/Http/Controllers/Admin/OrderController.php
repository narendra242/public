...
class OrderController extends Controller
{
    ...
    return view('admin.orders.pending_orders',compact('orders'),['head' => 'Pending Order List']);
 ...
 public function PendingOrdersDetails($order_id){
    ...
    $order = Order::with('district','user')->where('id',$order_id)->first();
    $orderItem = OrderItem::with('product')->where('order_id',$order_id)->orderBy('id','DESC')->get();
    return view('admin.orders.pending_orders_details',compact('order','orderItem'),['head' => 'Pending Order Detail']);
...
public function PendingToConfirm($order_id){
    ...
    return redirect()->route('admin.processing-orders')->with($notification);
       ...
       public function ShippedToDelivered($order_id){
        ...
        Product::where('id',$item->product_id)->update(['quantity' => DB::raw('quantity-'.$item->qty)]);
        } 
...
$pdf = PDF::loadView('admin.orders.order_invoice',compact('order','orderItem','general'))->setPaper('a4')->setOptions([
'tempDir' => public_path(),
'chroot' => public_path(),
]);
...
$del_record = Order::find($order_id);
$get_items= OrderItem::where('order_id', '=', $order_id);
if(!empty($del_record)){
$del_record->delete();
$get_items->delete();
